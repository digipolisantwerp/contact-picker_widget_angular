...
export class DemoLocalPage implements OnInit {
  ...
  picker1: ContactPickerValue = {
    id: "-2",
    name: "Hendrik Conscience",
  } as ContactPickerValue;
  ...
  ngOnInit() {
    this.http.get("./assets/antwerpenaars.json").subscribe((response) => {
      let counter = 0;
      this.listOfPeople1 = (response as Array<any>).map((v) => {
        return { id: (counter++).toString(), name: v } as ContactPickerValue;
      });
    });
    this.http.get("./assets/suskeenwiske.json").subscribe((response: any) => {
      let counter = 0;
